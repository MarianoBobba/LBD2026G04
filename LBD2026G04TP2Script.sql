...
-- 1. Dado un cliente, listar todos sus equipos, mostrando el número de serie, nombre y
-- descripción del modelo, nombre de la marca y nombre del tipo de equipo.
...
INNER JOIN Equipos e 
    ON c.idClientes = e.clientesId
INNER JOIN Modelos m 
    ON e.modelosId = m.idModelos
INNER JOIN Marcas ma 
    ON m.marcasId = ma.idMarcas
INNER JOIN TiposDeEquipos t 
    ON m.tiposId = t.idTiposDeEquipos
WHERE c.idClientes = 1;

-- 2. Dado un equipo, listar sus órdenes de trabajo entre un rango de fechas, mostrando la
-- falla, diagnóstico, apellido y nombre del cliente y el trabajo realizado (acciones técnicas).
...
SELECT 
    ...
    ot.diagnostico,
    c.nombre AS cliente,
    at.fecha AS fechaAccion,
    at.descripcion AS trabajoRealizado
FROM OrdenesDeTrabajo ot
INNER JOIN Clientes c
    ON ot.clientesId = c.idClientes
INNER JOIN AccionesTecnicas at
    ON ot.idOrdenesDeTrabajo = at.ordenesDeTrabajoId
WHERE ot.equiposId = 1
AND ot.fechaIngreso BETWEEN '2026-01-01' AND '2026-12-31';

-- 3. Idem anterior incluyendo los repuestos utilizados.
...
INNER JOIN AccionesTecnicas at
    ON ot.idOrdenesDeTrabajo = at.ordenesDeTrabajoId
INNER JOIN ConsumoDeRepuestos cr
    ON at.idAccionesTecnicas = cr.idAccionesTecnicas
INNER JOIN Repuestos r
    ON cr.idRepuestos = r.idRepuestos
WHERE ot.equiposId = 1
AND ot.fechaIngreso BETWEEN '2026-01-01' AND '2026-12-31';

-- 4. Idem anterior incluyendo las alarmas.
...
SELECT 
    ...
    at.fecha AS fechaAccion,
    at.descripcion AS trabajoRealizado,
    a.descripcion AS alarma,
    a.fechaProgramada,
    a.fechaRealizada,
    ea.nombre AS estadoAlarma
FROM OrdenesDeTrabajo ot
INNER JOIN Clientes c
    ON ot.clientesId = c.idClientes
INNER JOIN AccionesTecnicas at
    ON ot.idOrdenesDeTrabajo = at.ordenesDeTrabajoId
LEFT JOIN Alarmas a
    ON ot.equiposId = a.equiposId
LEFT JOIN EstadosAlarmas ea
    ON a.estadosId = ea.idEstadosAlarmas
WHERE ot.equiposId = 1
AND ot.fechaIngreso BETWEEN '2026-01-01' AND '2026-12-31';

-- 5. Hacer un ranking con los equipos que más órdenes de trabajo generaron entre un rango
-- de fechas.

SELECT 
    e.idEquipos,
    e.numeroSerie,
    COUNT(ot.idOrdenesDeTrabajo) AS cantidadOT
FROM Equipos e
INNER JOIN OrdenesDeTrabajo ot
    ON e.idEquipos = ot.equiposId
WHERE ot.fechaIngreso BETWEEN '2026-01-01' AND '2026-12-31'
GROUP BY e.idEquipos, e.numeroSerie
ORDER BY cantidadOT DESC;

-- 6. Hacer un ranking con los repuestos más utilizados en los trabajos realizados entre un
-- rango de fechas.

SELECT 
    r.nombre,
    SUM(cr.cantidad) AS totalUtilizado
FROM Repuestos r
INNER JOIN ConsumoDeRepuestos cr
    ON r.idRepuestos = cr.idRepuestos
INNER JOIN AccionesTecnicas at
    ON cr.idAccionesTecnicas = at.idAccionesTecnicas
WHERE at.fecha BETWEEN '2026-01-01' AND '2026-12-31'
GROUP BY r.idRepuestos, r.nombre
ORDER BY totalUtilizado DESC;

-- 7. Hacer un ranking con los técnicos que más trabajos realizaron entre un rango de fechas.

SELECT 
    u.nombre,
    u.apellido,
    COUNT(at.idAccionesTecnicas) AS trabajosRealizados
FROM Usuarios u
INNER JOIN AccionesTecnicas at
    ON u.idUsuarios = at.tecnicoId
WHERE at.fecha BETWEEN '2026-01-01' AND '2026-12-31'
GROUP BY u.idUsuarios, u.nombre, u.apellido
ORDER BY trabajosRealizados DESC;

-- 8. Crear una vista con la funcionalidad del apartado 2.

CREATE VIEW vista_historial_ot AS
SELECT 
    ot.numeroOT,
    ot.fechaIngreso,
    ot.fechaSalida,
...
SELECT *
FROM vista_historial_ot
WHERE equiposId = 1
AND fechaIngreso BETWEEN '2026-01-01' AND '2026-12-31';

-- 10. Realizar una vista que considere importante para su modelo. También dejar escrito el
-- enunciado de la misma.

-- La siguiente vista permite visualizar el historial completo de mantenimiento de cada equipo, 
-- incluyendo cliente, modelo, técnico y estado actual de la orden de trabajo.

CREATE VIEW vista_mantenimiento_completo AS
SELECT 
    e.numeroSerie,
    c.nombre AS cliente,
    m.nombre AS modelo,
    ...
    ot.fechaIngreso,
    ot.fechaSalida,
    est.nombre AS estadoOT,
    u.nombre AS tecnicoNombre,
    u.apellido AS tecnicoApellido
FROM Equipos e
INNER JOIN Clientes c
    ON e.clientesId = c.idClientes
INNER JOIN Modelos m
    ON e.modelosId = m.idModelos
INNER JOIN Marcas ma
    ON m.marcasId = ma.idMarcas
INNER JOIN OrdenesDeTrabajo ot
    ON e.idEquipos = ot.equiposId
INNER JOIN EstadosOT est
    ON ot.estadosId = est.idEstadosOT
INNER JOIN Usuarios u
    ON ot.tecnicoId = u.idUsuarios;
